    ...
    `}>
            ...
            <div className="flex-1 p-5 lg:p-6 flex flex-col relative">
                ...
                <div className="flex items-start justify-between mb-6">
                    <div>
                        <h3 className="text-slate-200 font-bold text-sm mb-1 truncate max-w-[200px]">
                            <span className="text-emerald-400 mr-2">#{displayIndex}</span>
                            {item.originalMeta.name}
                        </h3>
                ...
                {/* Controls Grid */}
                <div className={`grid grid-cols-2 md:grid-cols-3 gap-4 mb-6 transition-all duration-300 ${isProcessing || isDownloading ? 'opacity-60 grayscale pointer-events-none' : ''}`}>
                ...
                {/* Advanced Inputs */}
                <div className={`space-y-4 pt-4 border-t border-slate-800/50 flex-1 transition-all duration-300 ${isProcessing || isDownloading ? 'opacity-60 pointer-events-none' : ''}`}>
                ...
                <div className="mt-6 pt-4 border-t border-slate-800 flex items-end justify-between relative z-0">
                    ...
                    <div className="flex gap-2">
                        ...
                        {(isIdle || isSuccess) && (
                            <button onClick={handleDownload} disabled={isDownloading} className={`flex items-center gap-2 px-6 py-3 rounded-xl text-xs font-bold uppercase tracking-widest shadow-lg transition-all transform active:scale-95 ${isSuccess ? 'bg-emerald-500 hover:bg-emerald-400 text-white shadow-emerald-500/20' : 'bg-slate-700 hover:bg-slate-600 text-slate-300'}`}>
