...
export const VoiceAssistant: React.FC<VoiceAssistantProps> = ({
  ...
  // When modals change (user closes one manually), we tell the AI silently
  // so it doesn't try to close an already closed window later.
  useEffect(() => {
    if (isActive && sessionRef.current) {
      sessionRef.current.then((s: any) => {
        // Send as text input (system event)
        s.sendRealtimeInput([{ 
